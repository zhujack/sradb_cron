...
echo -e "\n\n\n\n#####################################################################################################" >>  ../sra_update.log
Today=`date -j +%Y%m%d`
echo -e "Updating SRAdb on $Today\n" >>  ../sra_update.log

## download NCBI_SRA_Metadata_Full only, which shoud be available once a month
cd /Users/zhujack/bin/cron_jobs/sra/download
NCBI_SRA_Metadata_newest="NCBI_SRA_Metadata_${Today}.tar.gz"
wget -qN ftp://ftp-trace.ncbi.nih.gov/sra/reports/Metadata/${NCBI_SRA_Metadata_newest} >> ../sra_update.log || exit 1
echo -e "Downloaded ${NCBI_SRA_Metadata_newest} on $Today \n" >> ../sra_update.log;
gtar --overwrite -zxf ${NCBI_SRA_Metadata_newest} -C xml/
echo -e "Uncompressed ${NCBI_SRA_Metadata_newest} to xml folder\n" >> ../sra_update.log;
## mv SRA_Accessions and SRA_Run_Members
mv xml/SRA_Run_Members xml/SRA_Accessions .
...
# for f in {20..30}
# do
#     wget https://ftp.ncbi.nlm.nih.gov/sra/reports/Metadata/NCBI_SRA_Metadata_202311${f}.tar.gz
# done
#
#
# for f in NCBI_SRA_Metadata_2023*.tar.gz
# do
#     echo $f
#     gtar --overwrite  -zxf $f -C xml/
# done
...
${mysql_dir}/mysql -h localhost -u zhujack -pmicroarray sra < ../scripts/prepare_sra.sql >> ../sra_update.log  2>&1

## rebuild table SRA_Accessions
${mysql_dir}/mysqlimport --local --ignore-lines=1 -h localhost -u zhujack -pmicroarray  sra --columns=Accession,Submission,Status,Updated,Published,Received,Type,Center,Visibility,Alias,Experiment,Sample,Study,Loaded,Spots,Bases,Md5sum,BioSample,BioProject,ReplacedBy SRA_Accessions >> ../sra_update.log  2>&1

# controlled_access 7,771,159
# public 95,061,685

## rebuild table SRA_Run_Members
${mysql_dir}/mysqlimport --local --ignore-lines=1 -h localhost -u zhujack -pmicroarray  sra --columns=Run,Member_Name,Experiment,Sample,Study,Spots,Bases,Status,BioSample SRA_Run_Members >> ../sra_update.log  2>&1
...
/Applications/XAMPP/bin/php scripts/sra_parser.php  >> sra_update.log  2>&1
...
mysql -h localhost -u zhujack -pmicroarray sra < scripts/add_sra_records.sql >>  sra_update.log  2>&1

##################################################
## generate sra_new contents from other tables and update the sra_new
echo -e "\n\n############################################" >>  sra_update.log
echo "## Generate sra_new.csv @ `date` \n" >>  sra_update.log
/usr/local/bin/R --no-save < scripts/sra_new.R  >>  sra_update.log  2>&1
chmod 777 sra_new.csv

echo -e "\n\n############################################" >>  sra_update.log
echo "## import sra_new.csv @ `date` \n" >>  sra_update.log
${mysql_dir}/mysqlimport --local --ignore-lines=1 --fields-enclosed-by='"' --fields-terminated-by=","  -h localhost -u zhujack -pmicroarray sra sra_new.csv 
...
${mysql_dir}/mysql -h localhost -u zhujack -pmicroarray sra < scripts/sra_mv.sql  >>  sra_update.log  2>&1
...
# fi
#
# ${mysql_dir}/mysqldump -uzhujack -pmicroarray -hlocalhost sra col_desc data_block experiment run sample sra SRA_Accessions SRA_Run_Members study submission | gzip > SRAdb.mysqldump.gz
